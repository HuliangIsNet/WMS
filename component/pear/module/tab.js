layui.define(['jquery', 'element','lang'], function (exports) {
	"use strict";
	...
	pearTab.prototype.render = function (opt) {
		...
		$("body .layui-tab[lay-filter='" + option.elem + "'] .layui-tab-title").on("contextmenu", "li", function (e) {
			...
			var menu = "<ul><li class='item' id='" + option.elem + "closeThis'>'"+$.i18n.prop('关闭当前')+"'</li><li class='item' id='" + option.elem +
				"closeOther' >'"+$.i18n.prop('关闭其他')+"'</li><li class='item' id='" + option.elem + "closeAll' >'"+$.i18n.prop('关闭全部')+"'</li></ul>";
	...
	pearTab.prototype.addTab = function (opt) {
		var title = '';
		if (opt.close) {
			title += '<span class="pear-tab-active"></span><span class="able-close title" >'+
			$.i18n.prop(opt.title)+'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		} else {
			title += '<span class="pear-tab-active"></span><span class="disable-close title" >'
			+$.i18n.prop(opt.title)+
	...
	pearTab.prototype.addTabOnly = function (opt, time) {
		var title = '';
		if (opt.close) {
			title += '<span class="pear-tab-active"></span><span class="able-close title" >' +$.i18n.prop(opt.title)+'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		} else {
			title += '<span class="pear-tab-active"></span><span class="disable-close title">'+$.i18n.prop(opt.title)+'</span><i class="layui-icon layui-unselect layui-tab-close">ဆ</i>';
		}
	...
	function createTab(option) {
		...
		$.each(option.data, function (i, item) {
			console.log($.i18n.prop(item.title));
			var TitleItem = '';
			...
			if (item.close) {
				// 当 前 选 项 卡 可 以 关 闭
				TitleItem += '<span class="able-close title" >'+$.i18n.prop(item.title)+'</span>';
			} else {
				// 当 前 选 项 卡 不 允 许 关 闭
				TitleItem += '<span class="disable-close title" >'+$.i18n.prop(item.title)+'</span>';
			}
		...
		title += '</ul>';
		content += '</div>';
		control += '<dd id="closeThis"><a href="#" data-i18n-text="关闭当前"></a></dd>'
		control += '<dd id="closeOther"><a href="#" data-i18n-text="关闭其他"></a></dd>'
		control += '<dd id="closeAll"><a href="#" data-i18n-text="关闭全部"></a></dd>'
