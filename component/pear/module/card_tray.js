layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	card.prototype.initOptions = function (opt) {
		this.option = $.extend(false, {}, defaultOption, opt);
	...
	function createComponent(elem,linenum,data) {
		console.log(linenum,data.count);
	...
	function createCard(elem, linenum, item, no) {
		var line = 12 / linenum;
		var card =
			'<div id=' + item.id + ' onclick="cardTableCheckedCard(' + elem + ',this)" class="layui-col-md' + line + ' ew-datagrid-item" data-index="' + no+'" data-number="1"> <div class="project-list-item"> <div class="project-list-item-cover" style="backgroud-color:#009688 !important;"  ><label class="tuopan">'+item.title+'</label></div> <div class="project-list-item-body"><div class="layui-carousel" id="carouse_'+item.id+'" lay-filter="carouse_'+item.id+'"> <div carousel-item=""><div style="text-align: center;">'+item.remark+'</div><div>'+item.remark+'</div></div></div> </div > </div > </div > '
		return card;
	}
