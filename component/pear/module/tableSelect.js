layui.define(['table', 'jquery', 'form', 'dropdown', 'common', 'apis'], function (exports) {
    "use strict";
    ...
    tableSelect.prototype.render = function (opt) {
        ...
        opt.toolName = opt.toolName || 'tool';
        opt.toolBack = opt.toolBack || function () { };
        //最小宽度
        ...
        elem.off('click').on('click', function (e) {
            ...
            if (opt.searchType == 'more') {
                $.each(opt.searchList, function (index, item) {
                    tableBox += '<input style="display:inline-block;width:100px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + item.searchKey + '" placeholder="' + item.searchPlaceholder + '" autocomplete="off" class="layui-input" />&nbsp;&nbsp;';
                });
            } else {
                tableBox += '<input style="display:inline-block;width:100px;height:30px;vertical-align:middle;margin-right:-1px;border: 1px solid #C9C9C9;" type="text" name="' + opt.searchKey + '" placeholder="' + opt.searchPlaceholder + '" autocomplete="off" class="layui-input">';
            }
            ...
            table.on('tool(' + tableName + ')', function (obj) {
                //获取到相关的存储信息
                var data = [];
                common.get({ trayId: obj.data.trayId }, apis.GetListByTrayId, function (res) {
                    layui.each(res, function (i, item) {
                        var q=parseInt(item.quantity);
                        var l= parseInt(item.lockQuantity);
                        console.log(q);
                        console.log(l);
                        var s=q-l;
                        data.push({ title: item.materialName + '(' + s + ')' });
                    });
                });
                //opt.toolBack(obj);
                var that = this;
