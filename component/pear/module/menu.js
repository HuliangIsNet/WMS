layui.define(['table', 'jquery', 'element'], function (exports) {
	...
	function getData(url) {
		...
		if ((new URL(url)).searchParams.toString().length> 0) {
		...
		$.each(option.data, function (i, item) {
			...
			if (item.type == 0) {
				...
				content += '<a  href="javascript:;" menu-type="' + item.type + '" menu-id="' + item.id + '" href="' + href +
					'" ' + target + '><i class="' + item.icon + '"></i><span data-i18n-text='+ item.title+'></span></a>';
			} else if (item.type == 1) {
				content += '<a class="' + className + '" menu-type="' + item.type + '" menu-url="' + item.href + '" menu-id="' +
					...
					'"></i><span data-i18n-text='+ item.title+'></span></a>';
			}
		...
		$.each(option.data, function (i, item) {
			...
			if (i === option.defaultMenu) {
				controlItem = '<li pear-href="' + item.href + '" pear-title="' + item.title + '" pear-id="' + item.id +
					'" class="layui-this layui-nav-item"><a href="#" data-i18n-text='+item.title+'></a></li>';
				menuItem = '<ul  pear-id="' + item.id + '" lay-filter="' + option.elem +
					'" class="layui-nav arrow layui-nav-tree pear-nav-tree">';

				controlPe += '<li class="layui-nav-item"><a class="pe-title" href="javascript:;" data-i18n-text='+item.title+' ></a>';

				controlItemPe += '<dd  pear-href="' + item.href + '" pear-title="' + item.title + '" pear-id="' + item.id +
					'"><a href="javascript:void(0);" data-i18n-text='+item.title+' ></a></dd>';
			} else {

				controlItem = '<li  pear-href="' + item.href + '" pear-title="' + item.title + '" pear-id="' + item.id +
					'" class="layui-nav-item"><a href="#" data-i18n-text='+item.title+'></a></li>';
